        ...
        if (!this.bufflonEntity.isSitting() && this.bufflonEntity.isFollowingOwner()) {
            if (--this.timeToRecalcPath <= 0) {
                ...
                if (!this.navigator.moveTo(this.owner, this.followSpeed)) {
                    if (!this.bufflonEntity.isLeashed() && !this.bufflonEntity.isPassenger()) {
                        if (!(this.bufflonEntity.distanceToSqr(this.owner) < 144.0D)) {
                            ...
                            for (int l = 0; l <= 4; ++l) {
                                for (int i1 = 0; i1 <= 4; ++i1) {
                                    if ((l < 1 || i1 < 1 || l > 3 || i1 > 3) && this.canTeleportToBlock(new BlockPos(i + l, k - 1, j + i1))) {
                                        this.bufflonEntity.moveTo((double) ((float) (i + l) + 0.5F), (double) k, (double) ((float) (j + i1) + 0.5F), this.bufflonEntity.getYRot(), this.bufflonEntity.getXRot());
                                        this.navigator.stop();
                                        return;
                                    }
                                }
