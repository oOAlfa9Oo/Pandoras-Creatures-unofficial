    ...
    public static void register() {
        for (PCBiomeFeatureCatalog.FeatureDefinition definition : PCBiomeFeatureCatalog.definitions()) {
            BiomeModifications.addFeature(
                    ...
                    ResourceKey.create(Registries.PLACED_FEATURE, ResourceLocation.parse(definition.featureId()))
            );
        }
    }
    ...
    private static Predicate<BiomeSelectionContext> toBiomeSelector(List<String> biomeSelectors) {
        if (biomeSelectors.size() == 1 && biomeSelectors.getFirst().startsWith("#")) {
            TagKey<Biome> tag = TagKey.create(
                    Registries.BIOME, ResourceLocation.parse(biomeSelectors.getFirst().substring(1)));
