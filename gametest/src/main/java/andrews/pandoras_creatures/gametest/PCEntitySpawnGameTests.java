...
public final class PCEntitySpawnGameTests {
    ...
    private static void assertSpawns(GameTestHelper helper, Mob mob, String name) {
        PCGameTestAssertions.assertTrue(helper, mob != null, name + " should spawn");
        PCGameTestAssertions.assertTrue(helper, mob.isAlive(), name + " should be alive after spawning");
        PCGameTestAssertions.assertTrue(helper, mob.level() == helper.getLevel(), name + " should be added to the test level");
        helper.succeed();
    }
