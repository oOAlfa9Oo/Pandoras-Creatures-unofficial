...
public final class EndPrisonStructureGameTests {
    ...
    public static void structureTemplatesLoadWithRealSize(GameTestHelper helper) {
        ...
        Optional<StructureTemplate> ship = level.getStructureManager().get(net.minecraft.resources.Identifier.parse(andrews.pandoras_creatures.world.structures.end_prison.EndPrisonBehaviorRules.VANILLA_SHIP_TEMPLATE));
    ...
    public static void generatedShipUsesOfficialProximity(GameTestHelper helper) {
        ...
        Holder.Reference<Structure> endPrisonHolder = structures.get(PCStructureIds.id(PCStructureIds.END_PRISON))
                .orElse(null);
        helper.assertTrue(endPrisonHolder != null, "End Prison must exist for ship proximity validation");
        if (endPrisonHolder == null) {
            return;
        }
        Structure endPrison = endPrisonHolder.value();
        ...
        for (int attempt = 0; attempt < 128 && generatedShip == null; attempt++) {
            ...
            StructureStart candidateStart = endPrison.generate(
                    endPrisonHolder, Level.END, end.registryAccess(), generator, generator.getBiomeSource(),
    ...
    public static void naturallyPlacedEndPrisonContainsEndTroll(GameTestHelper helper) {
        ...
        Holder.Reference<Structure> endPrisonHolder = structures.get(PCStructureIds.id(PCStructureIds.END_PRISON))
                .orElse(null);
        helper.assertTrue(endPrisonHolder != null, "End Prison must be present in the runtime structure registry");
        if (endPrisonHolder == null) {
            return;
        }
        Structure endPrison = endPrisonHolder.value();
        ...
        helper.assertTrue(!end.getChunkSource().getGeneratorState().getPlacementsForStructure(endPrisonHolder).isEmpty(),
